...
const PropertyList: React.FC = () => {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-surface-secondary text-app tracking-wider font-sans antialiased">
      ...
      <div className="px-6 md:px-12 py-6 bg-surface-secondary border-b border-neutral-200/50 sticky top-[64px] z-40">
        <div className="flex gap-3 max-w-5xl mx-auto w-full">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full h-12 bg-surface-secondary border border-neutral-200/60 rounded-sm pl-10 pr-4 text-xs tracking-wide focus:outline-none focus:border-neutral-900 transition-all"
            />
          </div>
          <button
            onClick={() => setShowFilters(!showFilters)}
            className={`size-12 rounded-sm flex items-center justify-center transition-all border ${showFilters ? 'btn-primary-theme text-white border-neutral-950' : 'bg-surface text-neutral-500 border-neutral-200/60 hover:border-neutral-900 hover:text-app'}`}
          >
          ...
          <div className="mt-6 pt-6 border-t border-neutral-200/60 animate-fade-in space-y-5 max-w-5xl mx-auto w-full">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  className="w-full h-11 bg-surface border border-neutral-200/60 rounded-sm text-xs font-medium tracking-wide px-3 focus:outline-none focus:border-neutral-900"
                >
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 h-12 rounded-sm border border-neutral-200/60 text-[10px] font-semibold uppercase text-neutral-500 tracking-[0.3em] hover:border-neutral-900 hover:text-app transition-colors"
              >
                Limpiar
              </button>
              <button
                onClick={handleApplyFilters}
                className="flex-[2] h-12 rounded-sm btn-primary-theme text-white text-[10px] font-semibold uppercase tracking-[0.3em] hover:bg-neutral-700 transition-all"
              >
                Aplicar y Buscar
              </button>
            </div>
      ...
      <div className="flex-1 max-w-5xl mx-auto w-full">
        {showFilters ? (
          ...
          <div className="flex flex-col items-center justify-center py-32 animate-fade-in">
            ...
            <h3 className="text-app font-medium text-sm uppercase tracking-[0.3em]">Configurando filtros</h3>
          ...
          <>
            <div className="flex items-center justify-between px-6 md:px-12 pt-8 pb-2">
              <h3 className="text-app font-medium text-xs uppercase tracking-[0.3em]">
              ...
              <div className="flex border border-neutral-200/60 rounded-sm overflow-hidden">
                <button onClick={() => setViewMode('grid')} className={`size-8 flex items-center justify-center transition-colors ${viewMode === 'grid' ? 'btn-primary-theme text-white' : 'text-neutral-400 hover:text-app'}`}>
                  <span className="material-symbols-outlined text-lg">grid_view</span>
                </button>
                <button onClick={() => setViewMode('list')} className={`size-8 flex items-center justify-center transition-colors border-l border-neutral-200/60 ${viewMode === 'list' ? 'btn-primary-theme text-white' : 'text-neutral-400 hover:text-app'}`}>
                ...
                filteredProperties.map(property => {
                  ...
                  return (
                    <div
                      ...
                      className={`group bg-surface border border-neutral-200/60 rounded-sm overflow-hidden cursor-pointer transition-all duration-500 hover:border-neutral-900 ${viewMode === 'list' ? 'flex flex-col sm:flex-row' : ''}`}
                    >
                      <div className={`${viewMode === 'list' ? 'h-48 sm:h-auto sm:w-2/5' : 'h-56'} relative overflow-hidden btn-primary-theme`}>
                        <img
                          src={
                            getImageUrl(
                        ...
                        {property.featured && (
                          <div className="absolute top-3 left-3 z-20 size-7 bg-surface/95 backdrop-blur-sm rounded-full flex items-center justify-center shadow-sm">
                            <span
                              className="material-symbols-outlined text-[15px] text-app"
                        ...
                        {statusLabel && (
                          <div className="absolute top-0 right-0 w-60 h-60 overflow-hidden z-20 pointer-events-none">
                            <div className="absolute top-[90px] right-[-58px] w-[365px] rotate-45 btn-primary-theme text-white text-center py-1 text-[10px] font-semibold uppercase tracking-[0.3em] shadow-md">
                              {statusLabel}
                            </div>
                          </div>
                        )}
                      ...
                      <div className="p-5 flex-1 flex flex-col justify-between">
                        <div>
                          <div className="flex justify-between items-start mb-1 gap-2">
                            <h4 className="text-app font-medium text-sm uppercase tracking-wide line-clamp-1">{property.title}</h4>
                            <span className="text-app font-semibold text-sm whitespace-nowrap">{formatPrice(property.price)}</span>
                          </div>
